...
const SocialLogin = ({ handleLogin }) => {
  ...
  // Initialize Facebook SDK with modern approach
  useEffect(() => {
  ...
  const handleFacebookAuthResponse = async (authResponse) => {
    try {
      ...
      const res = await axios.post(`${"process.env.NEXT_PUBLIC_API_URL"}/auth/facebook-login`, {
        accessToken,
        userID
      });
      ...
      setError(error.message || 'Failed to authenticate with the server');
      console.error('Server authentication error:', error);
    }
  };
  const handleFacebookLogin = async () => {
    if (!isFBSDKLoaded) {
      setError('Facebook SDK is not loaded yet. Please try again.');
      return;
    }
    try {
      setLoading(true);
      ...
      window.FB.login((response) => {
        if (response.status === 'connected') {
          ...
          setError('Please authorize this application to continue.');
        } else {
          setError('Could not log in to Facebook.');
        }
        setLoading(false);
      }, {
      ...
      {loading ? (
        ...
        <div className="space-y-3 mt-3">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 text-white py-2 px-4 rounded hover:opacity-90 transition-opacity"
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
