  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (response.data.message) {
        ...
        Swal.fire("Error", "Failed to generate OTP. Please try again.", "error");
      }
    } catch (error) {
      Swal.fire("Error", error.response?.data?.message || "Error generating OTP", "error");
    } finally {
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      if (response.data.status) {
        ...
        Swal.fire("Error", response.data.message || "Error logging in.", "error");
      }
    } catch {
          ...
          <div className="login-content">
            ...
            <form onSubmit={(e) => e.preventDefault()} className="login-form">
              {isOtpSent ? (
                <>
                  <p className="otp-message">
                    You're almost there! We have sent an OTP to <span style={{ fontWeight: "600" }}>{email}. <FaEdit style={{ cursor: "pointer" }} onClick={() => setIsOtpSent(false)} />
                    </span>
                  </p>
                  ...
                  {canResend&&<button onClick={handleSendOtp} className="btn" style={{margin:'auto'}} >
                    Resend OTP
                  </button>}
              ...
              <div className="button-container">
                {isOtpSent ? (
                  <button onClick={handleEmailLogin} className="btn">Sign Up</button>)
                 : (
                  <div>
                  <button onClick={handleSendOtp} className="btn" style={{ marginBottom:"15px",margin:'15px auto'}}>Send OTP</button>
